...
@login_required
def index(request):
    return HttpResponseRedirect(reverse("modoboa_admin:domain_list"))


@login_required
@user_passes_test(
    lambda u: u.has_perm("modoboa_admin.view_domains")
    or u.has_perm("modoboa_admin.view_mailboxes")
)
def _domains(request):
    ...
    if page is None:
        ...
        context["rows"] = _render_to_string(
            request, "modoboa_admin/domains_table.html", {
...
@login_required
@ensure_csrf_cookie
def domains(request, tplname="modoboa_admin/domains.html"):
    if not request.user.has_perm("modoboa_admin.view_domains"):
        if request.user.has_perm("modoboa_admin.view_mailboxes"):
            return HttpResponseRedirect(
                reverse("modoboa_admin:identity_list")
            )
...
@login_required
@permission_required("modoboa_admin.view_domains")
@reversion.create_revision()
...
@login_required
@permission_required("modoboa_admin.delete_domain")
...
def domain_statistics(request):
    ...
    return render(request, "modoboa_admin/domain_statistics.html", context)
